...
2. **A prompt *version tag* is recorded per LLM call, for reproducibility of a specific run's output.** Every `PromptSet` returned by the registry carries a `promptVersion` field — the short git commit hash of the prompt folder at load time (computed once at process start via `git rev-parse --short HEAD -- src/prompts/<key>/`, cached for the process lifetime, not re-computed per call). This value is written into `state.metrics` per module call (see `15-cost-tracking-system.md`) so that, months later, "which prompt version produced this specific published article" is an answerable question without needing a database — it's a join between the run's logged `promptVersion` and `git log`.
