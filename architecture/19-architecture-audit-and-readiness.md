...
| `promptVersion` (the repository HEAD short commit hash, a repository-wide value — not a per-prompt-folder hash, per the correction recorded in `13-prompt-management-system.md`) depends on the prompt files being inside the same git repository the pipeline runs from. | Correct for v1's single-repo layout. Flagged in `13-prompt-management-system.md` implicitly via its git-based design — if prompts ever move to a separately-deployed remote store (the same document's own localization-scale scenario), `promptVersion`'s computation needs to change from a `git rev-parse` call to whatever that store's own versioning primitive is. This is a small, isolated change (one function inside the registry), not a redesign. Note also that, as implemented, `promptVersion` reflects the whole repository's state at load time, not a given prompt folder's own last-changed commit — a distinction worth preserving even after any future remote-store migration. |
